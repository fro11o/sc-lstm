...
class SCLSTMCell(Layer):
    ...
    def build(self, input_shape):
        ...
        self.control_kernel = self.add_weight(
                shape=(input_dim, self.control_units),
                name='control_kernel',
                initializer=self.kernel_initializer,
                regularizer=self.kernel_regularizer,
                constraint=self.kernel_constraint)
        self.control_recurrent_kernel = self.add_weight(
                shape=(self.units, self.control_units),
                name='control_recurrent_kernel',
                initializer=self.recurrent_initializer,
                regularizer=self.recurrent_regularizer,
                constraint=self.recurrent_constraint)        
        self.mapping_kernel = self.add_weight(
                shape=(self.control_units, self.units),
                name='mapping_kernel',
                initializer=self.recurrent_initializer,
                regularizer=self.recurrent_regularizer,
                constraint=self.recurrent_constraint)        
    ...
    def call(self, inputs, states, training=None):
        ...
        d_tm1 = states[2]  # previous control state
        ...
        inputs_r = inputs
        x_r = K.dot(inputs_r, self.control_kernel)
        h_tm1_r = h_tm1
        r = self.recurrent_activation(
                    x_r + K.dot(h_tm1_r, self.control_recurrent_kernel))
        d = r * d_tm1
        c += self.activation(K.dot(d, self.mapping_kernel))
    ...
    def get_config(self):
        ...
        base_config = super(SCLSTMCell, self).get_config()
